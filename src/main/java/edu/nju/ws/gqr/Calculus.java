...
public class Calculus {
    ...
    Calculus(String calculusName, List<String> baseRelationNames, int identity, List<Integer> converseTable,
    ...
    final boolean baseRelationsAreSerial() {
        return baseRelationsSerial;
    }
    ...
    Relation encodeRelation(String s) {
        ...
        for (String name : names) {
            ...
            if (relationId == -1) {
                System.err.printf("Warning: unknown relation: \"%s\" \n", name);
                continue;
            }
            r.set(relationId);
        }
    ...
    boolean checkCompositionTable() {
        ...
        for (int i = 0; i < getNumberOfBaseRelations(); i++) {
            for (int j = 0; j < getNumberOfBaseRelations(); j++) {
                ...
                Relation r2 = getBaseRelationComposition(getBaseRelationConverse(j), getBaseRelationConverse(i));
    ...
    private boolean decideBaseRelationsSerial(List<List<Relation>> compositionTable) {
    ...
    private Relation buildIdentityRelation(int id) {
        Relation r = new Relation();
        r.set(id);
        return r;
    }

    private Relation buildUniversalRelation(int size) {
        Relation r = new Relation();
    ...
    public int size() {
        return this.baseRelationNames.size();
    }
